...
export const Card: React.FC<CardProps> = ({ word, className, wrongCount, onRemove }) => {
                    ...
                    <div className="flex flex-col items-center justify-center p-8 text-center h-full relative overflow-y-auto">
                        ...
                        {wrongCount !== undefined && wrongCount > 0 && (
                            <div className="absolute bottom-4 left-0 right-0 text-center">
                                <p className="text-red-500 text-xs font-bold bg-red-50 inline-block px-3 py-1 rounded-full">
                                    Wrong {wrongCount} time{wrongCount !== 1 ? 's' : ''}
                                </p>
                            </div>
                        )}

                        {onRemove && (
                            <div className={clsx("transition-opacity duration-300 absolute top-4 left-4 z-10", isFlipped ? "opacity-0 pointer-events-none" : "opacity-100")}>
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        if (window.confirm("Remove this word from Wrong Answer Note?")) {
                                            onRemove();
                                        }
                                    }}
                                    className="p-3 bg-red-50 text-red-500 rounded-full hover:bg-red-100 transition-colors"
                                    title="Remove from Wrong Answer Note"
                                >
